...
async function getFirstStudentWithNoScores() {
  ...
  return students.find((s) => {
    ...
    return s.subjects.some((subjName) => {
      const code =
        Object.keys(SUBJECT_MAPPING).find(
          (k) => SUBJECT_MAPPING[k] === subjName,
        ) || subjName;
...
function handleEnterCass() {
  getFirstStudentWithNoScores().then((student) => {
    if (student) {
      window.location.href = `EditStudCASS.html?id=${student.id}`;
    } else {
      alert("All registered students have complete CASS scores.");
    }
  });
}
